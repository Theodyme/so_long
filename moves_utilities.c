...
void	replace_asset(t_mlx *vars)
{
	if (vars->lvl->map[vars->ppos_y][vars->ppos_x] == '0')
		mlx_put_image_to_window(vars->mlx, vars->win, vars->assets->bg->img, (vars->ppos_x * 20), (vars->ppos_y * 20));
		...
		mlx_put_image_to_window(vars->mlx, vars->win, vars->assets->start->img, (vars->ppos_x * 20), (vars->ppos_y * 20));
...
int			move_up(t_mlx *vars)
{
	...
	replace_asset(vars);
	vars->ppos_y -= 1;
	mlx_put_image_to_window(vars->mlx, vars->win, vars->assets->chara->img, (vars->ppos_x * 20), (vars->ppos_y * 20));
	vars->mv += 1;
	return (0);
}
